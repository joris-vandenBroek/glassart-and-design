...
- `db/schema.sql` is the source of truth for the MySQL schema (30 tables: `klanten`, `medewerkers`, `sessions`, `passwordResetTokens`, catalog lookup tables `segmenten`/`stijlen`/`onderwerpen`/`materiaalsoorten`/`materialen`/`maten`/`prijsgroepen`/`prijsmatrix`, `kunstenaars`/`kunstenaarAfspraken`, `drukkers`/`drukkerZendingen`/`drukkerZendingBestellingen`, `kunstwerken` and its junction tables `kunstwerkSegmenten`/`kunstwerkMaterialen`/`kunstwerkMaten`/`kunstwerkStijlen`/`kunstwerkOnderwerpen`, `instellingen`, `schema_migrations`, `counters`, `bestelheaders`/`bestellines`/`bestelstatusHistorie`, `activiteitenlog`).
