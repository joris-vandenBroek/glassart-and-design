...
- `db/schema.sql` is the source of truth for the MySQL schema (30 tables: `klanten`, `medewerkers`, `sessions`, `passwordResetTokens`, catalog lookup tables `segmenten`/`stijlen`/`categorieen`/`materiaalsoorten`/`materialen`/`maten`/`prijsgroepen`/`prijsmatrix`, `kunstenaars`/`kunstenaarAfspraken`, `drukkers`/`drukkerZendingen`/`drukkerZendingBestellingen`, `kunstwerken` and its junction tables `kunstwerkSegmenten`/`kunstwerkMaterialen`/`kunstwerkMaten`/`kunstwerkStijlen`/`kunstwerkCategorieen`, `instellingen`, `schema_migrations`, `counters`, `bestelheaders`/`bestellines`/`bestelstatusHistorie`, `activiteitenlog`).
...
- Two `workflow_dispatch` GitHub Actions live in `.github/workflows/` (GitHub Pages deployment was retired once the app moved to server mode — Next.js server mode can't run as a static Pages site):
  - `deploy-naar-staging.yml` — builds in server mode and SFTP-deploys to the `staging.glassartanddesign.com` Node.js app on mijn.host. On every successful run (build + deploy + smoke check all pass) dispatched against `master`, it also computes the next `vN` version number, bakes it into the build as `NEXT_PUBLIC_APP_VERSION` (shown in the beheer header via `AppVersionLabel`), and pushes a git tag `vN` on the deployed commit. A `vN` tag means the build was uploaded successfully, staging automatically restarted (see below), and the smoke check confirmed that exact version is being served — not that a human has reviewed it; actually checking the new version on staging is still the developer's job before ever promoting it to production.
...
- Both deploy workflows trigger the Passenger restart themselves (an SFTP-uploaded `tmp/restart.txt` touch, followed by a warm-up request) and, on staging, confirm the new `NEXT_PUBLIC_APP_VERSION` is actually being served on `/nl/beheer` before tagging `vN` — a successful run means the new build is live, no manual restart needed. DirectAdmin's Node.js Selector has no API for `npm install`, only a UI button, so that one step is still manual: every deploy run ends with a `::notice::` reminder to click **Run NPM Install** in DirectAdmin, but only when `package.json`/`package-lock.json` changed in that deploy — `node_modules` is a symlink into a DirectAdmin-managed location the workflow can't upload to.
