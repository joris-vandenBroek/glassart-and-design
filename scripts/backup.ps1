...
#   3. bestandskopie de werkmap incl. alles wat git NIET ziet (.env, sleutels, config)
#
# Laag 3 is de eigenlijke reden dat dit script bestaat: GitHub dekt de code al,
# maar niet .env.local, .env.production.local, Beheer_users.txt,
# mijnhost-herstelcodes.txt, .claude/ en .superpowers/.
#
# Bewust NIET meegenomen:
#   node_modules/.next/out  herbouwbaar; samen 93% van de bestanden en 551 van de 568 MB
#   .git                    zie waarschuwing hieronder
#   nodetest-app            staat alleen op de netwerkschijf, geen projectonderdeel
#
# WAAROM .git NIET GESPIEGELD WORDT
# Robocopy is niet git-bewust. Bevat de kopie eigen commits die de bron niet heeft --
# een oude branch bijvoorbeeld -- dan wist /MIR die objecten zonder waarschuwing.
# Bij het opzetten bleek dat precies het geval: activiteitenlog en
# activiteitenlog-uitbreiding bestonden alleen in de kopie op de netwerkschijf.
# De bundel legt alle branches vast zonder dat risico.
#
# LET OP: gebruik het UNC-pad, niet L:. Schijfletters gelden per gebruikerssessie,
# dus een geplande taak die draait terwijl je niet bent ingelogd ziet L: niet.
#
# Draaien:  powershell -ExecutionPolicy Bypass -File scripts\backup.ps1
# Testen:   ... -File scripts\backup.ps1 -DryRun

param(
    [string]$Bron = 'C:\projecten\Glassart and design',
    [string]$Doel = '\\192.168.1.21\Transmission\Glassart and design',
    [switch]$DryRun
)

$ErrorActionPreference = 'Stop'

# Logboek naast het project, niet in %LOCALAPPDATA%: een geplande taak kan een
# andere omgeving hebben dan je eigen sessie, en dan schrijf je een logboek dat
# je nergens terugvindt. Dit pad volgt het project als het verhuist.
$logboek = Join-Path (Split-Path $Bron -Parent) 'glassart-backup.log'
...
$bundel     = Join-Path $Doel 'glassart-historie.bundle'
...
$argumenten = @(
    $Bron, $Doel,
    '/MIR',
    '/XD', "$Bron\.git", 'node_modules', '.next', 'out', '.worktrees', 'nodetest-app',
    '/XF', 'tsconfig.tsbuildinfo', '*.log', 'glassart-historie.bundle*',
