...
- [x] **Step 8: Retire GitHub Pages (partial) and deploy staging on the new architecture**

`deploy-pages.yml` workflow deleted and the `GITHUB_PAGES` env-var branch in `next.config.mjs` was already removed in Task 23. GitHub Pages itself (`gh api repos/.../pages`) is still enabled at the repo-settings level (`build_type: workflow`, `public: true`) — disabling that is a repo-settings change requiring explicit confirmation, not yet done.

Also done in this same pass: `staging.glassartanddesign.com` was deployed onto the same Node.js/MySQL architecture as production, but built **without** `MIJNHOST_BUILD` (so the launch-gated routes stay fully visible on staging) and **with** `NEXT_PUBLIC_ENVIRONMENT_LABEL=staging` (drives the yellow `StagingBanner`) and a staging-only `NEXT_PUBLIC_UPLOAD_SECRET` (so kunstwerk-photo uploads route to `uploads/kunstwerken-test/`, not the real `uploads/kunstwerken/` — `upload-kunstwerk-foto.php` already supported this staging/production secret split). Uses the existing `dv137864_staging` MySQL database (the same one dev/tests already point at), not a separate staging DB. Same DirectAdmin Node.js Selector pattern as production: `app.js` + `node app.js` startup, a fresh Node.js app entry (not the old `nodetest.glassartanddesign.com` one), zip built locally via the `System.IO.Compression.ZipArchive` method (see [[feedback_windows_zip_backslash_paths]]) and uploaded via File Manager since FTP data connections aren't reachable from this environment.

One real snag hit during this deploy: `npm install` via DirectAdmin's "Run NPM Install" button failed on the first attempt (`stderr.log` showed `Cannot find module 'next'`) with a generic DirectAdmin health-check error banner: `An error occured during installation of modules...`. A second attempt succeeded cleanly — worth knowing this button can fail transiently on a large `node_modules` install right after extracting a big zip.

Verified on staging afterward: `/nl/collecties` shows real content (not the gate) with the STAGING banner visible; `/api/segmenten` and `/api/instellingen/bedrijfsgegevens` return the real migrated data; `/nl/beheer` login works against the `dv137864_staging` MySQL database and shows real bedrijfsgegevens/catalog counts.

**Both Firebase projects (`glassart-and-design` and `glassart-and-design-staging`) were deleted by the user on 2026-07-28**, after directly verifying via the Firestore REST API (see [[feedback_firestore_direct_rest_access]]) that nothing of value remained: production's leftover `kunstwerken` (36 docs) and `activiteitenlog` (3 docs) were confirmed to be the app's auto-seed-on-empty demo data regenerated after the earlier deliberate wipe (all 36 created within the same 1.5-second window, generic names like "Abstract paneel 1"–"Wellness paneel 6"), not real business data — everything real had already been migrated to MySQL. Staging's Firestore project only held the pre-wipe backup copy, no longer needed once staging itself moved to MySQL. This closes out the Firebase-to-MySQL migration end-to-end: no Firebase project exists anywhere anymore, for any environment.

**Not yet done, flagged separately:**
- `deploy-naar-production.yml` and `deploy-naar-staging.yml` are both still stale (static-export FTP deploys, Firebase env vars) — rewriting them to build server-mode and deploy this same way (or better) is real follow-up work, not started.
- Production's real `NEXT_PUBLIC_UPLOAD_SECRET` value still needs to be set as `upload_secret` in the deployed `upload-server/config.php` before kunstwerk photo uploads work in production.
- Staging's `NEXT_PUBLIC_UPLOAD_SECRET` value (freshly generated for this deploy) needs to be set as `staging_upload_secret` (plus `staging_upload_public_base_url`) in that same deployed `upload-server/config.php`.
- `mail-server/config.php`'s `allowed_origins` needs a manual check that `https://staging.glassartanddesign.com` is present — the client-side order-confirmation and drukker emails (`CartPanel.tsx`, `VersturenNaarDrukkerDialog.tsx`) call `NEXT_PUBLIC_MAIL_ENDPOINT_URL` directly from the browser, so a missing origin there would silently CORS-block those calls from staging (not from production, which already works).
- Staging's Node.js app is still missing the `MAIL_SERVER_RESET_ENDPOINT_URL` environment variable (only `DB_*` vars were set) — the "wachtwoord vergeten" flow will silently no-op on staging until it's added, same gap Step 5 found and fixed for production.
- The old, now-unused `nodetest.glassartanddesign.com` Node.js app entry in DirectAdmin still exists and can be deleted.

**Update 2026-07-29:** GitHub Pages disabled at the repo-settings level (`gh api -X DELETE repos/.../pages`) with explicit user confirmation — `joris-vandenbroek.github.io/glassart-and-design/` no longer resolves. Step 8 is now fully complete. All remaining `NEXT_PUBLIC_FIREBASE_*` leftovers were also removed from the codebase: `.env.local`, `deploy-naar-production.yml`, `.gitignore`'s `firebase-service-account*.json` rule, the now-unrunnable `scripts/migrate-firebase-data.ts` (deleted, along with its npm script and the `firebase-admin` dependency), and stale code comments that still referenced Firestore-rules/`useFirestoreCollection`. No reference to Firebase or Firestore remains anywhere in the app.
