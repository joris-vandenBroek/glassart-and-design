...
al live staat en veilig is gebleken. Pas op `xl:` (≥1280px, elk normaal extern beeldscherm
of volledig-breed browservenster) groeit hij naar `560px`:

```
grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_560px]
```
...
(~600px). Door pas bij `xl:` te verbreden, blijft die tussenmaat ongemoeid en profiteert
alleen het gangbare "normaal browservenster op een gewoon scherm"-geval van de bredere,
realistischere preview.
...
extra ruimte op brede schermen, precies waar de `xl:`-kolom die nodig heeft. Bij `560px`
preview + 1400px modal houdt het formulier ~768px over, vrijwel gelijk aan de ~760px die
het nu al heeft bij `320px` preview + 1152px modal.
...
- `480px` is bewust ruim onder de beoogde `560px` (comfortabele marge) en ruim boven de
  ...
  preview op `lg:` (320px kolom) keurig 1-koloms, en springt hij pas op `xl:` (560px
  kolom) om.
...
- `KunstwerkenSection.test.tsx`: nieuwe test die bevestigt dat de grid-wrapper de
  `xl:grid-cols-[minmax(0,1fr)_560px]`-class draagt.
