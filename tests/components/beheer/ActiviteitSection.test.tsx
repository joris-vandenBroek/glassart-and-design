...
describe('ActiviteitSection', () => {
  ...
  it('shows Dutch labels for the new Stijl and Categorie activiteit types', () => {
    renderSection([
      { id: 'log-3', type: 'stijl_toegevoegd', actorEmail: 'paul@glassartanddesign.com', actorNaam: 'Paul', timestamp: null },
      { id: 'log-4', type: 'categorie_verwijderd', actorEmail: 'paul@glassartanddesign.com', actorNaam: 'Paul', timestamp: null },
    ]);
    expect(screen.getByTestId('data-table-row-log-3')).toHaveTextContent('Stijl toegevoegd');
    expect(screen.getByTestId('data-table-row-log-4')).toHaveTextContent('Categorie verwijderd');
  });
});
